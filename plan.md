...
Latest addition: prepare reusable entity/relation annotations for a future literature encoder, using the same source-grounded KG extraction. The shared annotation manual is now in `configs/extraction_schema.yaml` and injected into both models' system prompts. See [annotation design and training-label boundaries](docs/P4_ANNOTATION_DESIGN.md). Candidate export is implemented; current trial cases remain diagnostic, with no gold-label or encoder-training completion claim. This user instruction supersedes the earlier exclusion of preparing training data, without authorizing a full-corpus restart.

Latest user instruction (2026-09-23): **BOUNDED_ACCURACY_TRIAL_AUTHORIZED** for `prescient.kimi-k3` and `prescient.glm-5.3-flash`. Full production remains stopped. Extraction was stopped and all generated P4 extraction artifacts/response caches were deleted. Source preparation and `.env` are preserved. Scientific accuracy acceptance is reopened; follow [the research and repair decision](docs/P4_ACCURACY_RESEARCH.md). Use these two user-supplied aliases only for bounded comparison; no production restart or P5 evidence integration follows from older instructions below.

**Prepared:** 2026-09-21. **Updated:** 2026-09-23. **Status:** P4 stopped for entity/relation accuracy research and repair; scientific acceptance reopened; comprehensive corpus expansion remains required afterward.
...
**Current user decision, 2026-09-23:** Build and test P4 on the available selected papers before extending discovery. Validate deterministic extraction/verification, the bounded read-only assembly agent, existing parsers and Docling using local LangChain documentation. Then validate P5 graph-file export and extend the corpus automatically as specified in §16. This changes execution order, not the final manuscript scope. Missing PDFs remain pending and do not block other ready papers. This instruction supersedes historical blanket P4 stop boundaries below. Preserve `.env`; use `litellm:prescient.glm-5.3-flash`. No full-corpus extraction is claimed or authorized merely by a smoke run.
...
- Implement the XML/BioC and local Docling PDF paths required by the advisor corpus before live extraction of their outputs. Abstract-only processing stays explicitly incomplete for full-text coverage. For the current P4 implementation/validation assignment, pin and verify only the ready source subset; missing/blocked evidence stays pending. Complete comprehensive preparation and gap dispositions before final manuscript acceptance (§16). The user has selected LiteLLM GLM on this laptop.
...
| P4 | Structured extraction, verification, accepted/uncertain/rejected findings | Verified source subset for implementation/validation; full corpus closure remains pending | Historical estimate only | STOPPED_FOR_ACCURACY_REPAIR; generated extraction artifacts deleted — [research](docs/P4_ACCURACY_RESEARCH.md) |
...
**P4 runtime implementation decision, 2026-09-23:** reuse the already working pinned `langchain-openai` OpenAI-compatible adapter for this laptop's LiteLLM proxy and for OpenRouter, with explicit base URL, unmodified deployment alias and provider-isolated parameters. This supersedes the older ChatLiteLLM proxy-prefix/ChatOpenRouter constructor prescription below, not the provider/credential/routing contract. Groq remains a lazy `ChatGroq` adapter in its own pinned extra. SDK transport tests verify the exact proxy path and strict structured schema; live P4 capability remains unverified while proxy DNS is unavailable. The assembly SDK is pinned to `langchain==1.4.2`; its LangGraph dependencies are internal to `create_agent`.
...
**Goal:** convert verified ready batches into atomic source-grounded evidence artifacts, retaining support, negative/null results, and uncertainty. P3 parses documents; P4 extracts biological findings. The 2026-09-23 user assignment permits P4 implementation and bounded live validation on the verified ready subset before comprehensive corpus expansion. Pin source hashes and enforce model-specific call/token limits before inference. Extractor/verifier have no tools; only the specified read-only assembly agent has bounded tools.
...
The [evidence-assembly contract](docs/assignments/P4-evidence-assembly-agent.md) specifies inputs/outputs, six tools, middleware, states, persistence, validation and limits. It is required implementation work within the current P4 assignment. The user has selected LiteLLM GLM; concrete live validation bounds must be recorded before calls. Complete bundles bypass assembly.

Latest implementation detail (2026-09-23): `configs/extraction_schema.yaml` defines intermediate mention/observation types; `configs/kg_schema.yaml` defines the final 20-node/29-relationship vocabulary and evidence dimensions from the complete final synthesis and KG schema. Pydantic generates structured contracts and both models receive the extraction boundary in system instructions. Source-aligned cached PubTator entity candidates remain hints, not verified relations. The final claim-centered graph does not turn biological extraction predicates into Neo4j relationship labels. See [KG schema alignment](docs/KG_SCHEMA_ALIGNMENT.md).

**Lead accuracy review reopened, 2026-09-23:** entity typing/grouping and relation endpoint/direction defects invalidate the previous scientific readiness conclusion. The earlier engineering tests remain historical evidence; all extraction artifacts, including `extraction-e0f05381fe0b5e98`, have been deleted at user request. Repair schemas, normalization, atomic observations and field-level verification; evaluate precision, recall and direction on independently reviewed held-out source passages before production. Details and published sources are in [P4_ACCURACY_RESEARCH.md](docs/P4_ACCURACY_RESEARCH.md). Bounded paired GLM/Kimi trials have completed; neither overall accuracy nor a model winner is established. The new KG-alignment changes have offline fixture validation only.

The latest accuracy assignment supersedes the earlier fixed two-call design where decomposition requires additional bounded calls. Re-estimate a concrete validation allowance before any future inference. Completed provider/assembly/source-integrity infrastructure remains reusable; unchecked items below are reopened for scientific quality and renewed evidence, not instructions to rebuild everything.

### P4 execution order

- [x] **P4.1 Implement chat providers and configure selected roles.** Implement LiteLLM proxy, OpenRouter, and Groq via their LangChain integrations following §6.4.1. Resolve qualified extractor/verifier selectors; an omitted verifier selector inherits the extractor. Check constructor/proxy routing offline for all three, install/load only selected extras, and validate only selected credentials. Confirm the selected model's structured-output mode within the total run budget. Keep raw responses/metadata locally, whether or not optional tracing is enabled. Provider absence is a clear setup error; never silently use a different model or provider.
- [x] **P4.1a Implement bounded evidence assembly.** Follow the linked agent contract; implement read-only tools and source-part registry, structured proposal, host validation, durable call/token/cost ledger and review handoff. Preserve P3 artifacts and model-provider isolation. Test bypass, unresolved context, wrong source references, cross-asset linkage, limits/restarts and replay; agent output alone cannot clear essential semantics blockers.
...
- [ ] **P4.4 Extract and verify.** Use structured mention recognition, atomic observations, a source-only independent read, field verification and source coverage criticism (five base calls for a nonempty bundle). One missing-mention expansion can add a bounded call. Account separately for capability checks, retries and optional evidence assembly within the concrete cumulative allowance. Empty outputs now receive the independent source read and coverage audit; omission accuracy still requires independent evaluation before production. The user-authorized [automated critic/repair extension](docs/P4_CRITIQUE_AUTOMATION.md) adds one correction pass, optional bounded local context recovery and fresh verification: at most nine structured calls, or fourteen with context recovery, per bundle. Scientific validation remains open. Preserve raw/parsed responses, evidence, usage, uncertainty and remaining queue; no first-batch authorization implies full-corpus execution.
- [x] **P4.5 Deterministic source checks.** Validate quotes/offsets against stored canonical text and document hash; map entities through approved resolver; validate schema/enums and source IDs; flag missing or conflicting fields. The model cannot assign unchecked passage offsets, stable IDs, or a made-up PMID. For table findings, validate referenced cells and raw values, signs, inequalities, units and comparison direction against their own source assets and context. Python-derived numbers require a reproducible calculation record with input hashes, selection/parameters and code version; they must not be labelled as author-reported statistics. The LLM does not calculate statistics or repair malformed tables.
...
.venv/bin/python -m regkg extract --run <p3selection_key> --dry-run --assembly
.venv/bin/python -m regkg extract --run <p3selection_key> --live --assembly
...
NicheExpressionSignature --HAS_GENE_EFFECT {mean1, mean2, effect, fractions}--> Gene
...
EvidenceAssertion --SUPPORTED_BY_PASSAGE--> Passage --PART_OF--> Publication
TFNomination --NOMINATES_TF--> Gene
TFNomination --FOR_STATE/IN_CONTEXT/BASED_ON_PROGRAM/GENERATED_BY--> State/Context/Program/Run
TFNomination --USES_CLAIM--> RegulatoryClaim
TFNomination --USES_CHROMLINKER--> ChromLinkerObservation
```

Canonical vocabulary is now `configs/kg_schema.yaml`, following the final KG document. The former aliases HAS_GENE, GROUNDED_IN, FROM_PUBLICATION, NOMINATES and USES_PROGRAM map respectively to HAS_GENE_EFFECT, SUPPORTED_BY_PASSAGE, PART_OF, NOMINATES_TF and BASED_ON_PROGRAM. The former USES_EVIDENCE shortcut is represented by USES_CLAIM → HAS_EVIDENCE; retain per-nomination/per-assertion context_match, polarity, comparison basis and rule version in the file-first evidence-application records. Never promote relative annotations to global assertion properties. The reviewed ontology extension remains planned work, not an implicit expansion of this core vocabulary. Do not add arbitrary generated relationship types from LLM strings.
...
- [ ] The required §16 discovery expansion has run with explicit search saturation/truncation and per-lineage evidence-gap reports; no arbitrary top-10 or target paper-count ceiling.
...
- Discovery beyond the manuscript-relevant expansion specified in §16 may be assigned later. The §16 expansion and required four-lineage evidence-gap accounting are mandatory, not deferred.
...
## 16. Current next assignment and required follow-on work

**User decision, 2026-09-23: establish the working KG extraction pipeline first.** Build and validate P4 using current selected sources, then validate graph-file integration, then expand discovery/acquisition and run the same pipeline on new eligible evidence. This is one complete deliverable, not V1/V2 or a smaller finish line. The expanded crawl has not started.

### 16.1 Active: P4 implementation and bounded validation

**Execution checkpoint (2026-09-23):** [P4 handoff](docs/handoffs/P4.md) and [CONTINUE_HERE](docs/CONTINUE_HERE.md) are current. LiteLLM GLM/Kimi calls work. User-authorized repairs add pre-extraction source-grounded experiment frames, bounded context assembly, targeted observation patches, two-finding semantic review batches and finding-level dispositions separate from coverage. Final KG vocabulary is unchanged. Ninety-four focused extraction tests and 72 corpus/parser tests pass; bounded live regression validation is running. Reuse frozen parsed sources and completed calls. Full-corpus extraction remains stopped pending practical validation, not an independent-gold prerequisite. Acceptance checkboxes remain open.

Inputs: `mcandidates-c278c6625ff7c537`, parent corpus `litcorpus-68fea35a393fe816`, and reconciled selection `p3selection-89efefdd214ee16e`. See [P3 reconciliation](docs/handoffs/P3-selection-reconciliation.md). Current counts are 156 selected records, 115 papers with usable text, 1,133 verified ready bundles, and 41 outstanding main texts. These are starting inputs, not sufficient/comprehensive-corpus claims. The user accepts continuing with available sources while missing papers remain pending; this does not accept blocked evidence or close P3.

- [ ] Pin the current ready subset and source hashes in a P4 input manifest. Keep the 41 papers and all blocked parsing/context/attribution items in the coverage ledger. Never relabel them READY or excluded because text is unavailable.
- [ ] Implement all §10 P4 contracts using ordinary deterministic workflow functions, tool-free structured extractor/verifier calls, and the single bounded agent in [the assembly contract](docs/assignments/P4-evidence-assembly-agent.md). Use the local `reference/langchain-sdk-main/_index.md` documentation and verify APIs against installed pinned packages.
- [ ] Verify JATS/BioC, supplement readers and Docling with focused real-source and behavioral checks; reuse parsed source caches and model assets. Fix concrete defects without reacquiring/reparsing the whole corpus.
- [ ] Run a representative real-source validation batch (at most 10 papers/20 bundles), covering all four lineages when source-ready examples exist. Test negative/null outcomes, source errors and ambiguous context with clearly synthetic fixtures where real examples are absent. Record any unexercised live branch.
- [ ] Use the existing LiteLLM GLM selection, including inherited verifier/assembler selectors unless explicitly overridden. Validate structured output/tool compatibility, bounded retry/token accounting and replay. Use finite per-role/shared limits; the user's unlimited proxy quota is not evidence of zero dollar price. No OpenRouter fallback or `.env` replacement.
- [ ] Persist real accepted/uncertain/rejected and empty findings, claims, evidence assertions, exact spans, source/experiment context, usage and manifests. Acceptance requires faithful output and reproducible replay, not a minimum positive-finding count. Do not use a successful smoke batch as authorization to run every extraction batch.

### 16.2 Next: validate graph-file integration

After P4 validation/review, execute P5's offline export and validation against those real findings. Confirm stable identities, provenance, relationships and repeat-export behavior before corpus expansion. Neo4j loading remains the separate P5 command and review boundary; no graph writes in P4. A working export or small graph is implementation evidence, not manuscript completion.

### 16.3 Required next corpus work: automated comprehensive expansion

This is mandatory follow-on work before final acceptance, not the optional expansion in §15. There is no publication-year restriction and no fixed target of 156 papers.

- [ ] Replace the 10-result discovery ceiling with resumable pagination of the manuscript query scope, including existing synonym/gap queries. Record total hits, retrieved unique records, remaining pages, failed sources and provider-imposed limits per query. Do not silently call a truncated search complete. Review remaining coverage gaps across all four lineages and add explicit manuscript-relevant queries where needed; no universal PubMed crawl.
- [ ] Deduplicate publication identities before model screening, retaining verified preprint/journal links and source provenance. Reuse compatible completed screening results. Automatically screen/rerank new titles and abstracts in bounded structured LiteLLM batches, preserving include/background/uncertain/exclude reasons. Ranking schedules work; no hard top-k or exact-TF veto discards the tail. Missing abstracts/full text remain uncertain rather than evidence of irrelevance.
- [ ] Automatically fetch eligible selected/uncertain sources through authorized routes, parse new assets, screen available full text, follow needed supplement dependencies and rebuild source-grounded bundles. Acquisition eligibility must not depend on open-access availability. Only genuine unresolved download needs go to the actionable manual list; transient failures retain automated retry states. Manual PDFs are incrementally ingested without holding up available sources.
- [ ] Run validated P4 on newly eligible bundles with recorded corpus-wide limits; preserve successful source/model/prompt caches, uncertainty and negative findings. Update P5 graph files/imports idempotently without duplicate claims or independent-study inflation.
- [ ] Report final selected/available/processed/missing papers, per-lineage/regulator/claim support and counter-evidence, independent-study uncertainty, search coverage and unresolved gaps. These determine manuscript sufficiency; raw paper count does not. Complete P6 only after required work is done or has an explicit evidence-backed disposition.

Keep a single current progress ledger, immutable scientific provenance, small resumable checkpoints and only useful caches. Do not regenerate restored inputs, copy the corpus into parallel stores, or accumulate obsolete report snapshots. P3–P6 acceptance checkboxes stay open until their actual evidence is reviewed.
