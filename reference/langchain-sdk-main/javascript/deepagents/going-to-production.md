...
source: "https://docs.langchain.com/oss/javascript/deepagents/going-to-production"
category: "docs"
tags: [docs, javascript, deepagents, going-to-production]
---
...
* **User**: someone interacting with your agent. Memory and files can be private to a user or shared across users. Identity and authorization comes from your [auth layer](../../langsmith/auth.md).
...
For the full set of configuration options (custom Docker steps, store indexing, auth handlers, and more), see [application structure](../langgraph/application-structure.md).
...
#### [View example trace](https://smith.langchain.com/public/92d99101-7d19-46ac-8050-24d1cad6039b/r)
...
[LangSmith Deployments](../../langsmith/deployment.md) supports [custom authentication](../../langsmith/custom-auth.md) to establish user identity and [authorization handlers](../../langsmith/auth.md) to control access to resources like threads, assistants, and store namespaces. Authorization handlers run after authentication succeeds and can:
...
For a step-by-step tutorial, see [Make conversations private](../../langsmith/resource-auth.md). For a walkthrough, watch the [custom auth video](https://www.youtube.com/watch?v=DkNqgCz8cjE).
...
LangSmith's [role-based access control](../../langsmith/rbac.md) governs who on your team can deploy, configure, and monitor agents. This is separate from end-user authorization above.
...
Custom roles with granular permissions are available on Enterprise plans. See the [RBAC reference](../../langsmith/rbac.md) for the full permission model.
...
**OAuth via Agent Auth.** [Agent Auth](../../langsmith/agent-auth.md) provides a managed OAuth 2.0 flow. Configure an OAuth provider, and the agent can request tokens scoped to each user. On first use, the agent [interrupts](../langgraph/interrupts.md) execution and presents an OAuth consent URL. After the user authenticates, the agent resumes with a valid token. Tokens are stored and refreshed automatically.
...
**Credential injection for sandboxes.** If your agent runs code inside a [sandbox](#sandboxes) that calls external APIs, the [sandbox auth proxy](../../langsmith/sandbox-auth-proxy.md) can inject credentials into outbound requests automatically, so sandbox code never receives raw API keys. See [Managing secrets](#managing-secrets) for setup details.

**Workspace secrets.** For API keys shared across all users (for example your organization's LLM provider keys, search API keys), store them as [workspace secrets](../../langsmith/set-up-hierarchy.md#configure-workspace-settings) in LangSmith. See [Managing secrets](#managing-secrets) for details.
...
* **Use async middleware methods.** Custom [middleware](../langchain/middleware/custom.md) should implement async hooks (e.g., `abefore_agent` instead of `before_agent`).
* **Use async for external resource lifecycle.** Creating [sandboxes](#sandboxes) or connecting to [MCP servers](../langchain/mcp.md) involves network calls and should be awaited. This is why [graph factories](../../langsmith/graph-rebuild.md) that provision these resources are async.
...
* **Indefinite [interrupts](../langgraph/interrupts.md).** Human-in-the-loop workflows can pause for minutes or days and resume exactly where they left off.
* **[Time travel](../langgraph/use-time-travel.md).** Every checkpointed step is a snapshot you can rewind to, letting you replay from an earlier state if something goes wrong.
...
> [!TIP]
> [LangSmith Deployments](../../langsmith/deployment.md) configure a persistent checkpointer automatically. If you are self-hosting, see [persistence](../langgraph/persistence.md) for setup instructions.
...
You can also read and write to the store from your application code using the [Store API](../../langsmith/custom-store.md). See [Advanced usage](memory.md#advanced-usage) for examples.

For the full namespace factory API, see [namespace factories](backends.md#namespace-factories). For memory patterns like self-improving instructions and knowledge bases, see [long-term memory](memory.md).
...
* [StoreBackend](https://reference.langchain.com/javascript/deepagents/backends/StoreBackend): cross-thread storage that survives across conversations. Scope with a [namespace factory](backends.md#namespace-factories).
...
For the full list of backends and how to build custom ones, see [backends](backends.md).
...
If your agent needs to run code (not just read and write files), use a [sandbox](sandboxes.md). Sandboxes provide both a filesystem and an `execute` tool for running shell commands, all inside an isolated container. This isolation also protects your host: if the agent's code exhausts memory or crashes, only the sandbox is affected. Your server keeps running.
...
| -------------------- | ----------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------- |
| **Thread-scoped**    | [Thread](../../langsmith/use-threads.md) metadata | Fresh per conversation, cleaned up on TTL | A data analysis bot where each conversation starts clean             |
| **Assistant-scoped** | [Assistant](../../langsmith/assistants.md) config | Shared across all conversations           | A coding assistant that maintains a cloned repo across conversations |

> [!NOTE]
> The examples below use an async [graph factory](../../langsmith/graph-rebuild.md) instead of a static graph because the sandbox needs the `thread_id` or `assistant_id` to look up or create the correct sandbox. Graph factories don't receive a full `Runtime` (no `server_info` or `execution_info`); instead, accept a `RunnableConfig` and read `thread_id` and `assistant_id` from `config["configurable"]`. The factory is async because sandbox creation is an I/O-bound operation that requires per-run information only available at invocation time.
...
Because the `agent` variable is an async function (not a compiled graph), the server treats it as a [graph factory](../../langsmith/graph-rebuild.md) and calls it on each run, injecting the config. The factory looks up or creates the sandbox by name and returns a fresh agent graph wired to that sandbox.
...
* **Seed the sandbox before the agent runs**: upload user files, [skill](skills.md) scripts, configuration, or [persistent memories](memory.md) so the agent has what it needs from the start
...
For provider-specific file transfer examples, see [working with files](sandboxes.md#working-with-files). For provider setup, security, and lifecycle patterns, see the full [sandboxes guide](sandboxes.md).
...
The `${SECRET_KEY}` references resolve against secrets stored in your LangSmith [workspace settings](../../langsmith/set-up-hierarchy.md#configure-workspace-settings). Configure secrets there before creating a template that references them.

**Workspace secrets.** For API keys that don't need proxy-based injection (e.g., keys used by the agent server itself, not sandbox code), store them as [workspace secrets](../../langsmith/set-up-hierarchy.md#configure-workspace-settings) in LangSmith. These are available as environment variables at runtime for all agents in the workspace.
...
[Permissions](permissions.md) are declarative allow/deny rules that control which files and directories the agent can read or write. Use permissions to isolate the agent to a working directory, protect sensitive files, or enforce read-only memory. Rules are evaluated in declaration order, and the first matching rule wins.

### Fault tolerance

For rate limiting, retries, fallbacks, and error handling, see [Fault tolerance](fault-tolerance.md).
...
For the default Deep Agents middleware stack, see [Customization](customization.md#middleware). For additional LangChain prebuilt middleware (retries, fallbacks, PII detection, and more), see [Prebuilt middleware](../langchain/middleware/built-in.md).

## Frontend

Deep Agents use [`useStream`](../langchain/frontend/overview.md) to connect your UI to the agent backend. [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream) is a frontend hook (available for React, Vue, Svelte, and Angular) that streams messages, subagent progress, and custom state from your agent in real time.

Locally, `useStream` points at `http://localhost:2024`. In production, point it at your [LangSmith Deployment](../../langsmith/deployment.md) and configure reconnection so users don't lose progress if their connection drops.
...
For UI patterns specific to deep agents, such as subagent cards, todo lists, and custom state rendering, see the [frontend guide](frontend/overview.md).
