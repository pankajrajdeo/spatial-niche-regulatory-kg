...
category: "docs"
tags: [docs, javascript, deepagents, a2a]
---
...
Once your agents are running locally via `langgraph dev` or [deployed to production](../../langsmith/deployment.md), you can facilitate communication between them using the A2A protocol.
...
When multiple agents communicate over A2A, LangSmith can group all their [traces](../../langsmith/observability-concepts.md#traces) into a single [thread](../../langsmith/observability-concepts.md#threads), which gives you a unified view of the entire multi-agent conversation.
...
3. Agent Server maps the `contextId` to `thread_id` in LangSmith [metadata](../../langsmith/add-metadata-tags.md), so all turns appear in the same thread.
