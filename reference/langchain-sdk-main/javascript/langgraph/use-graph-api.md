...
description: "This guide demonstrates the basics of LangGraph's Graph API. It walks through state, as well as composing common graph structures such as sequences, branches, and loops. It also covers LangGraph's..."
source: "https://docs.langchain.com/oss/javascript/langgraph/use-graph-api"
category: "docs"
tags: [docs, javascript, langgraph, use-graph-api]
---
...
[State](graph-api.md#state) in LangGraph is defined using the `StateSchema` class. This provides a unified API that accepts [standard schemas](https://standardschema.dev/) (like [Zod](https://zod.dev/)) for individual fields along with special value types like `ReducedValue`, `MessagesValue`, and `UntrackedValue`.
...
Let's consider a simple example using [messages](graph-api.md#working-with-messages-in-graph-state). This represents a versatile formulation of state for many LLM applications. See our [concepts page](graph-api.md#working-with-messages-in-graph-state) for more detail.
...
Let's build an example graph with a single node. Our [node](graph-api.md#nodes) is just a TypeScript function that reads our graph's state and makes updates to it. The first argument to this function will always be the state:
...
Let's next define a simple graph containing this node. We use [`StateGraph`](graph-api.md#stategraph) to define a graph that operates on this state. We then use [`addNode`](graph-api.md#nodes) populate our graph.
...
| `user`        | `BaseUser \| null` | The authenticated user, if [custom auth](../../langsmith/custom-auth.md) is configured. |
...
To add a sequence of nodes, we use the `.addNode` and `.addEdge` methods of our [graph](graph-api.md#stategraph):
...
Our [nodes](graph-api.md#nodes) are just TypeScript functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
We will then use [addNode](graph-api.md#nodes) and [addEdge](graph-api.md#edges) to populate our graph and define its control flow.
...
We can now create the `StateGraph` with the above nodes. Notice that the graph doesn't have [conditional edges](graph-api.md#conditional-edges) for routing! This is because control flow is defined with `Command` inside `nodeA`.
