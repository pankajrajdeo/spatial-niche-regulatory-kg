...
category: "docs"
tags: [docs, javascript, langgraph, use-functional-api]
---
...
#### [View example trace](https://smith.langchain.com/public/8737a21a-1a44-47ce-b3bd-880a15fc7375/r)
...
The functional API supports [human-in-the-loop](interrupts.md) workflows using the [`interrupt`](https://reference.langchain.com/javascript/langchain-langgraph/index/interrupt) function and the `Command` primitive.
...
Note that we've paused with an [`interrupt`](https://reference.langchain.com/javascript/langchain-langgraph/index/interrupt) after `step_1`. The interrupt provides instructions to resume the run. To resume, we issue a [`Command`](interrupts.md#resuming-interrupts) containing the data expected by the `human_feedback` task.
