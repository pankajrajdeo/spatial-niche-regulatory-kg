...
source: "https://docs.langchain.com/oss/javascript/deepagents"
category: "docs"
tags: [docs, javascript, deepagents]
---
...
> **Interactive example:** [Open it in the original LangChain documentation](https://docs.langchain.com/oss/javascript/deepagents).
...
#### [View example trace](https://smith.langchain.com/public/7837efa6-ba5d-44f4-a88c-7cabaa4d18cb/r)
Open a public LangSmith run for this example.

See the [Quickstart](deepagents/quickstart.md) and [Customization guide](deepagents/customization.md) to get started building your own agents and applications with Deep Agents.

> [!TIP]
> Trace requests, debug agent behavior, and evaluate outputs with [LangSmith](https://smith.langchain.com?utm_source=docs\&utm_medium=cta\&utm_campaign=langsmith-signup\&utm_content=oss-deepagents-overview). Follow the [observability quickstart](../langsmith/observability-quickstart.md) to get set up. When ready for production, see [Going to production](deepagents/going-to-production.md) for LangSmith deployment options.
...
[`deepagents`](https://www.npmjs.com/package/deepagents) is a standalone library built on top of [LangChain](langchain.md)'s core building blocks for agents and using [LangGraph](langgraph.md)'s tooling for running agents in production.

[LangChain](langchain.md) is the framework that provides the core building blocks for your agents.
To learn more about the differences between LangChain, LangGraph, and Deep Agents, see [Frameworks, runtimes, and harnesses](concepts/products.md). For a side-by-side comparison with Anthropic's harness, see [Deep Agents vs. Claude Agent SDK](deepagents/comparison.md).

For building custom agents without these built-in capabilities, consider using LangChain's [`createAgent`](langchain/agents.md) or building a custom [LangGraph](langgraph/overview.md) workflow.
...
Pass custom functions, LangChain tools, or tools from any [MCP server](deepagents/tools.md#mcp-tools) with the `tools=` parameter. Deep Agents fully support the [Model Context Protocol (MCP)](langchain/mcp.md), letting you connect to databases, APIs, file systems, and more through a standard interface.

For more information on defining custom tools, using MCP servers, and the full list of built-in harness tools, see [Tools](deepagents/tools.md).

### Virtual filesystem access

The harness provides a configurable virtual filesystem which can be backed by different [pluggable backends](deepagents/backends.md): in-memory state, local disk, LangGraph store, composite routing, or a custom backend with [permission rules](deepagents/permissions.md) for read and write access.
...
| `execute`    | Run shell commands in the environment (available with [sandbox backends](deepagents/sandboxes.md) only)                                                                                                     |
...
<details>
<summary>Running without the default filesystem tools</summary>

To hide the filesystem tools listed above from the model, register a [harness profile](deepagents/profiles.md#harness-profiles) with `excluded_tools`:
...
Removing [`FilesystemMiddleware`](https://reference.langchain.com/javascript/deepagents/middleware/createFilesystemMiddleware) itself via `excluded_middleware` is intentionally rejected—it is required scaffolding in the [Deep Agents stack](deepagents/customization.md#deep-agents-stack). Use `excluded_tools` to hide only the model-visible tool surface and leave the middleware in place. To remove the `task` tool, see [Running without subagents](deepagents/subagents.md#running-without-subagents).
...
For more information, see [backends](deepagents/backends.md). To generate a durable repository wiki that agents can read from the filesystem, see [OpenWiki](../openwiki/overview.md).
...
Permissions do not apply to [sandbox backends](deepagents/sandboxes.md), which support arbitrary command execution via the `execute` tool. For custom validation logic, use [backend policy hooks](deepagents/backends.md#add-policy-hooks).

For the full rule structure, examples, and subagent inheritance, see [Permissions](deepagents/permissions.md).

### Code execution

Deep Agents supports code execution in two ways:

* [Sandbox backends](deepagents/sandboxes.md) expose an `execute` tool for shell commands in an isolated environment.
* [Interpreters](deepagents/interpreters.md) add an `eval` tool that runs JavaScript in a scoped QuickJS runtime.
...
For sandbox setup, providers, and file transfer APIs, see [Sandboxes](deepagents/sandboxes.md). For the QuickJS runtime and programmatic tool calling, see [Interpreters](deepagents/interpreters.md).

### Streaming

[Event streaming](deepagents/event-streaming.md) exposes agent runs as typed projections for messages, tool calls, values, and output. Deep Agents add `stream.subagents` so each delegated task gets its own handle with independent message, tool-call, and nested subagent streams.
...
For more information, see [Skills](deepagents/skills.md).
...
For configuration details and examples, see [Memory](deepagents/customization.md#memory). To generate a repository wiki that coding agents discover through `AGENTS.md`, see [OpenWiki](../openwiki/overview.md).
...
For configuration details, see [Context engineering](deepagents/context-engineering.md). For multimodal inputs and tool outputs, see [Multimodal](deepagents/multimodal.md).
...
For other providers, see [Middleware integrations](integrations/middleware.md) for available provider-specific caching middleware.
...
* UIs that stream progress from agent state (see [Todo list](deepagents/frontend/todo-list.md))
...
For configuration options and behavior details, see [To-do list](langchain/middleware/built-in.md#to-do-list).
...
* **Configurable strategy**: Use the [default `general-purpose` subagent](deepagents/subagents.md#default-subagent) (enabled by default) or define [custom subagents](deepagents/subagents.md#custom-subagents).
...
<details>
<summary>Running without subagents (no `task` tool)</summary>

To run an agent without the `task` tool, see [Running without subagents](deepagents/subagents.md#running-without-subagents). Do not try removing [`SubAgentMiddleware`](https://reference.langchain.com/javascript/deepagents/middleware/createSubAgentMiddleware) via `excluded_middleware`—that is intentionally rejected. Instead, disable the auto-added subagent via the [harness profile](deepagents/profiles.md#harness-profiles) and pass no synchronous subagents via `subagents=`. Async subagents are unaffected. See the [full stack](deepagents/customization.md#full-stack) for the complete ordering.

</details>

For more information, see [Subagents](deepagents/subagents.md).
...
For more information, see [Human-in-the-loop](deepagents/human-in-the-loop.md).

## Get started

#### [Quickstart](deepagents/quickstart.md)
Build your first deep agent

#### [Customization](deepagents/customization.md)
Learn about customization options

#### [Code](../deepagents/code/overview.md)
Use Deep Agents Code

#### [ACP](deepagents/acp.md)
Use deep agents in code editors with ACP

#### [Reference](https://reference.langchain.com/javascript/modules/deepagents.html)
