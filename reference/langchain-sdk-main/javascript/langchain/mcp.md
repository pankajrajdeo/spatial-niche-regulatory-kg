...
category: "docs"
tags: [docs, javascript, langchain, mcp]
---
...
MCP servers run as separate processes—they cannot access LangGraph runtime information like the [store](../langgraph/stores.md), [context](context-engineering.md), or agent state. Use `beforeToolCall` and `afterToolCall` hooks on `MultiServerMCPClient` to modify tool arguments, headers, or results:
