...
category: "docs"
tags: [docs, javascript, langchain, tools]
---
...
#### [View example trace](https://smith.langchain.com/public/59163065-33b5-4b9e-a8d0-dc882e31f97c/r)
...
> [!TIP]
> For production deployments, use a persistent store implementation like [`PostgresStore`](https://reference.langchain.com/javascript/langchain-langgraph-checkpoint-postgres/store/PostgresStore), `MongoDBStore`, or `RedisStore` instead of `InMemoryStore`. See the [memory documentation](../langgraph/add-memory.md) for setup details.
...
In LangChain, tools are used by agents (for example via [`create_agent`](https://reference.langchain.com/javascript/langchain/index/createAgent)) and tool error handling is configured through [middleware](middleware.md).

For LangGraph workflows, tool execution is handled by [`ToolNode`](https://reference.langchain.com/javascript/langchain-langgraph/prebuilt/ToolNode). See [ToolNode](../langgraph/workflows-agents.md#toolnode) for Graph API usage, including how tools can access the current graph state and run-scoped context.
...
For block types and provider-specific requirements, see [Multimodal messages](messages.md#multimodal).
...
#### [View example trace](https://smith.langchain.com/public/8f9bb682-fa0b-4264-8495-246703f0624b/r)
Open a public LangSmith run for this example.
...
For setup, transports, and authentication, see [Model Context Protocol (MCP)](mcp.md).
