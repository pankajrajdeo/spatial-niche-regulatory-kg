...
category: "docs"
tags: [docs, javascript, langchain, middleware, built-in]
---
...
#### `Field` — `(ClientTool | ServerTool | string)[]`
...
> [!TIP]
> The `fraction` conditions for `trigger` and `keep` (shown below) rely on a chat model's [profile data](../models.md#model-profiles) if using `langchain@1.1.0`. If data are not available, use another condition or specify manually:
>
> ```typescript
...
#### `Field` — `(string | StructuredToolInterface)[]`
...
#### `Field` — `(string | ClientTool | ServerTool)[]`
