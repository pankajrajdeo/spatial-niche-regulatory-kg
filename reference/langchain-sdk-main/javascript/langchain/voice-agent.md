...
source: "https://docs.langchain.com/oss/javascript/langchain/voice-agent"
category: "docs"
tags: [docs, javascript, langchain, voice-agent]
---
...
Voice agents are [agents](agents.md) that can engage in natural spoken conversations with users. These agents combine speech recognition, natural language processing, generative AI, and text-to-speech technologies to create seamless, natural conversations.
...
The agent stage processes text transcripts through a LangChain [agent](agents.md) and streams the response tokens. In this case, we stream all [text content blocks](messages.md#content-block-reference) generated by the agent.

### Key concepts

**Streaming Responses**: The agent uses [`stream_events(version="v3")`](streaming.md) with `stream.messages` to emit response tokens as they are generated, rather than waiting for the complete response. This enables the TTS stage to begin synthesis immediately.

**Conversation Memory**: A [checkpointer](short-term-memory.md) maintains conversation state across turns using a unique thread ID. This allows the agent to reference previous exchanges in the conversation.
...
For more on building agents with LangChain, see the [Agents guide](agents.md).
