...
You can also run OpenWiki inside Codex, Claude Code, OpenCode, or Cursor. See [Coding-agent integrations](integrations.md).
...
OpenWiki's durable output is Markdown (OKF), not a static HTML site. To explore the wiki in a browser, run [`openwiki visualize`](visualize.md). The command serves a viewer only on your local machine (`127.0.0.1`), or you can [export a static visualizer](visualize.md#export-a-static-site) for hosting.
