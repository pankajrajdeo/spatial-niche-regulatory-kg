...
category: "docs"
tags: [docs, langsmith, deployment-quickstart-da]
---
...
Create and deploy your first Managed Deep Agent: scaffold a project, configure the model and instructions, add search, test in [LangSmith Studio](studio.md), and deploy with the [`mda` CLI](python/managed-deep-agents-cli.md). Managed Deep Agents supplies the [Deep Agents harness](../deepagents/overview.md) and hosted runtime.

After this quickstart, the [tutorial](python/managed-deep-agents-tutorial.md) adds durable memory and a daily schedule on the same project.
...
> **Prompt:** Build a Managed Deep Agent with the quickstart
Create and deploy a Managed Deep Agent in this working directory by following the Managed Deep Agents quickstart.

## Step 1: Read the guide

Fetch and follow [https://docs.langchain.com/langsmith/managed-deep-agents-quickstart.md](managed-deep-agents-quickstart.md) as the source of truth for CLI commands, project layout, and deployment steps. Prefer the Python or TypeScript path that matches this project.
...
When you deploy, Managed Deep Agents syncs these instructions to [LangSmith Context Hub](python/managed-deep-agents-context-hub.md), where you can update them without redeploying the agent.
...
The agent name is also the default deployment name. For model concepts and provider options, see [Models](../langchain/models.md).
...
For more authored tools, see [Custom tools](python/managed-deep-agents-tools.md).
...
For more information, see [Develop locally with LangSmith Studio](python/managed-deep-agents-local-development.md).
