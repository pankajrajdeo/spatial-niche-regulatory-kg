...
Each bulk export is divided into multiple *runs*, where each run processes data for a [specific date partition](data-export.md#partitioning-scheme) (typically organized by day). Runs are processed independently, which enables:
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Infrastructure interruption** | [Deployments](deployment.md), server restarts, worker crashes                                                                                                                                                                           | Yes, automatically requeued with remaining retries. | None, jobs resume automatically.                                                                                             |
| **Run timeout**                 | Single run exceeds 4-hour limit                                                                                                                                                                                                                 | Yes, retried up to 20 times (subject to change).    | If persistent, narrow date range, add filters, or [limit the exported fields](data-export.md#limit-exported-fields). |
| **Workflow timeout**            | Entire export exceeds 72 hours                                                                                                                                                                                                                  | No                                                  | Reduce export scope (date range, filters) or break into smaller exports.                                                     |
| **Storage/destination errors**  | [Invalid credentials](data-export-destinations.md#credentials-configuration), [missing bucket](data-export-destinations.md#configuration-fields), [permission issues](data-export-destinations.md#permissions-required) | No                                                  | Fix destination configuration and create new export.                                                                         |
...
On [LangSmith Self-hosted](self-hosted.md), the concurrency limits are the defaults. To tune pod memory usage during bulk exports, configure the following environment variables on the `langsmith-backend` service:
...
1. **Check the export status**: Use the [`GET /api/v1/bulk-exports/{export_id}` endpoint](smith-api/bulk-exports/get-bulk-export.md) to retrieve the export details and status.
...
3. **Verify destination access**: Ensure your [destination bucket](data-export-destinations.md#configuration-fields) still exists and [credentials](data-export-destinations.md#credentials-configuration) are valid.
4. **Check run size**: If you see timeout errors, your date partitions may contain too much data. It may be helpful to [limit the exported fields](data-export.md#limit-exported-fields).
