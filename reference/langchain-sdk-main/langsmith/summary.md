...
* `runs: list[Run]`: A list of the full [Run](run-data-format.md) objects generated by the two experiments on the given example. Use this if you need access to intermediate steps or metadata about each run.
* `examples: list[Example]`: All of the dataset [Example](example-data-format.md) objects, including the example inputs, outputs (if available), and metadata (if available).
