...
* **[Fleet](index.md)** allows you to create, deploy, and manage AI agents directly within LangSmith with no code.
...
* **[Engine](../engine-overview.md)** finds recurring issues in a tracing project, diagnoses them against your source code, and proposes fixes. Engine requires Sandboxes, and is installed from [Engine on Self-hosted](../engine-self-hosted.md).
...
<details>
<summary>Salesforce OAuth provider</summary>
...
For the full walkthrough, see [Set up Slack on Self-hosted](slack-app.md#set-up-slack-on-self-hosted). It covers creating the Slack app, adding bot scopes, registering the provider, setting the redirect URI, and configuring Helm values.
...
Engine requires [Sandboxes](#enable-sandboxes), and it shares a deployment with [Insights](#enable-fleet-insights-and-chat) when that is also enabled. Unlike the other features on this page, Engine cannot run entirely inside your cluster: it uses LangSmith Intelligence, a LangChain-managed zero data retention service, for the model work that powers diagnosis and fixes.

For that reason, Engine's installation, egress, and data handling are documented together on one page. See [Engine on Self-hosted](../engine-self-hosted.md) for regional availability, prerequisites, Helm values, and verification steps.
