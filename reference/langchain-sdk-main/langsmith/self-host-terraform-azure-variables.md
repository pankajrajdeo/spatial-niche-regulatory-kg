...
category: "docs"
tags: [docs, langsmith, self-host-terraform-azure-variables]
---
...
For the end-to-end install, refer to the [deploy guide](self-host-terraform-azure-deploy.md). For how the modules fit together, refer to the [architecture reference](self-host-terraform-azure-architecture.md).
...
| `clickhouse_source` | `in-cluster` | no       | `in-cluster` (dev/POC only) or `external` for [LangChain Managed ClickHouse](langsmith-managed-clickhouse.md), recommended for production.    |
