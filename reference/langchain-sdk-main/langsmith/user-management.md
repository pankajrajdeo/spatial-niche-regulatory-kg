...
To send a user to a specific page after sign-in, add a URL-encoded `redirect_to` path to the login URL:

```text
https://smith.langchain.com/sso/login/<slug>?redirect_to=%2Fo%2F<organization-id>%2Fprojects%2Fp%2F<project-id>%2Fr%2F<run-id>
```

The value must decode to a path on the same host, starting with `/`. In the example above it decodes to `/o/<organization-id>/projects/p/<project-id>/r/<run-id>`, so the user lands on that run. A value pointing at another host is rejected, and the user lands on the LangSmith home page instead.

Encode the path. An unencoded path that contains its own query string or fragment is truncated at the first `&` or `#`, which sends the user to an incomplete destination.

> [!NOTE]
> Most links do not need this parameter. Opening a LangSmith URL directly, then signing in through **SSO** with a work email, already returns the user to that page. Use `redirect_to` when the link must start SAML sign-in itself, such as a link generated by a script or posted for a team that signs in through a single identity provider.
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td>
          <code>
            {`${protocol_11 === false ? "" : "https://"}${prefix_11 || "api.smith"}.langchain.com${suffix_11 || ""}`}
          </code>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td>
          <code>
            {`${protocol_11 === false ? "" : "https://"}eu.${prefix_11 || "api.smith"}.langchain.com${suffix_11 || ""}`}
          </code>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td>
          <code>
            {`${protocol_11 === false ? "" : "https://"}apac.${prefix_11 || "api.smith"}.langchain.com${suffix_11 || ""}`}
          </code>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td>
          <code>
            {`${protocol_11 === false ? "" : "https://"}aws.${prefix_11 || "api.smith"}.langchain.com${suffix_11 || ""}`}
          </code>
        </td>
      </tr>
    </tbody>
