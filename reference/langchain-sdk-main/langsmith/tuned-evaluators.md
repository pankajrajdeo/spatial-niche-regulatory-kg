...
> [!NOTE]
> LangChain Tuned Evaluators are in **[beta](release-stages.md)**. Their interface and behavior may change before general availability.
...
* An [organization admin](rbac.md#organization-admin) must enable LangChain Tuned Evaluators for the organization.
* The source must be a tracing project that uses [threads](threads.md). The Perceived Error (Tuned) evaluator cannot run on individual runs.
* Thread traces must contain supported [message lists](online-evaluations-multi-turn.md#prerequisites). At least one non-empty user message must be followed by a non-empty assistant message.
...
Like other online evaluators, LangChain Tuned Evaluators upgrade evaluated traces to extended retention. This can increase trace storage charges. For current rates, see [LangSmith pricing](pricing-plans.md). For details about retention upgrades and trace charges, see [data retention auto-upgrades](usage-and-billing.md#data-retention-auto-upgrades).
...
Check the inputs and outputs of the project's most recent threads. Their top-level `messages` fields must contain message lists in a [supported format](online-evaluations-multi-turn.md#prerequisites), and at least one user message must be followed by an assistant message.
...
* [Set up multi-turn online evaluators](online-evaluations-multi-turn.md)
* [Trace with threads](threads.md)
