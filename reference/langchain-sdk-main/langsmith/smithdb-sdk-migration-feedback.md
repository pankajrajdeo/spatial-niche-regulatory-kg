...
These methods add runs to annotation queues, share runs publicly, and create feedback. For deprecation dates, minimum SDK versions, and the agent prompt that applies to every method, see [Migrate to SmithDB-backed SDK methods](smithdb-sdk-migration.md).
...
> [!NOTE]
> This method stays on the existing client, not the new `runs` v2 client, so the [Exceptions](smithdb-sdk-migration.md#exceptions) table does not apply—error handling is unchanged.
...
| ------------------------------------ | ------------------------------------- | ----------------------------------------------------- |
| `.bodyOfRunsUuidArray(List)` | *(removed)*                           | Legacy body; run IDs only                             |
...
#### Java
`createByKey()` returns `List`—the same shape `create()` returned, with `id()`, `queueId()`, `runId()`, `addedAt()`, and `lastReviewedTime()`.
...
* [Dataset experiment runs](smithdb-sdk-migration-experiments.md)
