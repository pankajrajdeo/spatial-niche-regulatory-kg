...
A workspace is a logical grouping of users and resources within an organization. Workspaces are commonly used to isolate teams or business units, providing separation between projects and their associated resources. A workspace separates trust boundaries for resources and access control. Users are granted permissions at the workspace level, which determine their access to resources in that workspace, including tracing projects, datasets, annotation queues, and prompts. For details on setup, see the [setup guide](set-up-hierarchy.md#set-up-a-workspace) and for details on permissions see [Workspaces (RBAC)](#workspace-roles-rbac).

We recommend creating a separate workspace for each team within your organization. To organize resources even further, you can use [Applications](#applications) to group resources within a workspace. For guidance on different workspace organization models based on your team's isolation requirements, refer to [Workload isolation](workload-isolation.md).
...
Applications are built on top of [resource tags](#resource-tags) and can be used to control resource access using [ABAC](organization-workspace-operations.md#access-policies).
...
\*\* Self-hosted installations may enable workspace-level invites of users to the organization via a feature flag. For details, refer to the [self-hosted user management docs](self-host-user-management.md).

### Resource tags

Resource tags allow you to further segregate resources within a workspace for use with [ABAC](organization-workspace-operations.md#access-policies). Each tag is a key-value pair that you can assign to a resource.
...
> [!NOTE]
> To see how to create a service key or Personal Access Token, see the [setup guide](create-account-api-key.md)

### Organization roles

Organization roles are distinct from the [Enterprise feature workspace RBAC](#workspace-roles-rbac) and are used in the context of multiple [workspaces](#workspaces). Your organization role determines your workspace membership characteristics and your [organization-level permissions](organization-workspace-operations.md).

The organization role selected also impacts workspace membership as described here:

* [Organization Admin](rbac.md#organization-admin) grants full access to manage all organization configuration, users, billing, and workspaces.
  * An Organization Admin has `Admin` access to all workspaces in an organization.
* [Organization User](rbac.md#organization-user) may read organization information but cannot execute any write actions at the organization level. An Organization User may create [Personal Access Tokens](#personal-access-tokens-pats).
...
* [Organization Viewer](rbac.md#organization-viewer) is equivalent to Organization User, but **cannot** create Personal Access Tokens. (for self-hosted, available in Helm chart version 0.11.25+).
...
>
> See [security settings](manage-organization-by-api.md#security-settings) for instructions on how to disable PAT creation for the entire organization.

For more information on setting up organizations and workspaces, refer to the [organization setup guide](set-up-hierarchy.md#organization-roles) for more information.
...
* [Workspace Admin](rbac.md#workspace-admin) has full access to all resources within the workspace.
* [Workspace Editor](rbac.md#workspace-editor) has full permissions except for workspace management (adding/removing users, changing roles, configuring service keys).
* [Workspace Viewer](rbac.md#workspace-viewer) has read-only access to all resources within the workspace.

[Organization admins](rbac.md#organization-admin) can also create/edit custom roles with specific permissions for different resources.

You can manage roles under **Organization Settings** > **Members and roles** and select the **Roles** tab.

* For comprehensive documentation on roles and permissions, refer to the [Role-based access control](rbac.md) guide.
* For more details on assigning and creating roles, refer to the [User Management](user-management.md) guide.
* For a comprehensive list of required permissions along with the operations and roles that can perform them, refer to the [Organization and workspace reference](organization-workspace-operations.md).
...
* **Automation rules**: An [automation rule](rules.md#create-a-rule) with retention extension enabled matches any run within a trace. Matching a single run upgrades the entire trace, not just that run. A rule whose [item type](rules.md#set-the-item-type-to-runs-or-threads) is **Threads** upgrades every trace in the matched thread, not only the most recent one.
...
> **Interactive content:** [View this section in the original documentation](administration-overview.md).
...
For *spend* limits on evaluator runs specifically, refer to [Track and limit evaluator spend](evaluator-spend.md).
...
* **[Release policy](release-versions.md)**: Learn about the self-hosted release channels, cadence, and version numbering.
