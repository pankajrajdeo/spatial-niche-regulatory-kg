...
* You have likely deployed LangSmith without setting up SSO. LangSmith requires SSO to manage users and API keys. You can find more information on setting up SSO in the [configuration section.](self-host-sso.md)
...
You can find more information on setting up SSO in the [configuration section.](self-host-sso.md)

### *Can I use external storage services?*

* You can configure LangSmith to use external versions of all storage services. In a production setting, we strongly recommend using external storage services. Check out the [configuration section](self-hosted.md) for more information.
...
* Subscription Metrics and Operational Metadata (if not running in offline mode)
  ...
  * See [Egress](self-host-egress.md) for more information
...
1. Remove the user from the organization (see [manage users](set-up-hierarchy.md#manage-users))
2. Change their email address in the IdP
3. Have them login to LangSmith again via SAML SSO - this will trigger the usual [JIT provisioning](user-management.md#just-in-time-jit-provisioning) flow with their new email address
...
Update the user's group membership in your IdP. The changes will be synchronized to LangSmith according to the [role precedence rules](user-management.md#role-precedence).
...
Yes. If your identity provider supports syncing alternate fields to the `displayName` group attribute, you may use an alternate attribute (like `description`) as the `displayName` in LangSmith and retain full customizability of the identity provider group name. Otherwise, groups must follow the specific naming convention described in the [Group Naming Convention](user-management.md#group-naming-convention) section to properly map to LangSmith roles and workspaces.

You can also [configure a custom separator](user-management.md#configure-custom-separator) (e.g., `-`, `_`, `&`) instead of the default colon (`:`) to accommodate identity providers with restrictions on group name characters.
...
Yes! You can use the [development version of Agent Server](local-dev-testing.md) to run the backend locally.
