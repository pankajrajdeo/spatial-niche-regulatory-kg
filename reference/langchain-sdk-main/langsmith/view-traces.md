...
From a tracing project, use the **Threads**, **Traces**, or **Runs** tabs to change what appears in the table. To narrow it to specific rows, [apply a filter](filter-traces.md). Click into any row to open the side panel.

The side panel is organized around [threads](observability-concepts.md#threads) as the primary unit of navigation. Instead of treating each [run](observability-concepts.md#runs) as an isolated object, the UI keeps the surrounding conversation visible so you can understand where a run fits in the agent's broader execution.
...
* [**Messages**](#messages-view) (**beta**): The conversation layer. Scan the [trajectory](observability-concepts.md#trajectories) as inputs, outputs, reasoning, tool calls, and subagent activity. Use this to find where to look. Press `M` to switch to this view.
...
> [!NOTE]
> The Messages tab is disabled for threads that don't have any renderable messages. The Messages view is in **[beta](release-stages.md)**—the side panel defaults to the Details view.
...
The Messages view is in **[beta](release-stages.md)**. The side panel defaults to the [Details view](#details-view).

Use the Messages view to scan the full [trajectory](observability-concepts.md#trajectories) and identify unexpected behavior, such as a bad tool result, an unexpected subagent handoff, or a latency spike, before drilling into a specific run.
...
* `LS_MESSAGE_VIEW_EXCLUDE`: Exclude an individual run from the Messages view. Import the constant from `langsmith` (Python and JS), or use the literal string `"ls_message_view_exclude"`. For code examples, refer to [Exclude runs from the Messages view](messages-view-integrations.md#exclude-runs-from-the-messages-view).
...
For the integrations that set this metadata automatically, refer to [Messages view integrations](messages-view-integrations.md).
...
Setting `run_type="llm"` on a run causes the Details view to render token counts and latency for that run. For the full message format specification, refer to [Log an LLM trace](log-llm-trace.md).

Tool messages are auto-expanded when a run's `run_type` is `tool`.

Setting `run_type="retriever"` on a run causes the Details view to render each retrieved document with its contents and metadata inline. For the required return format, refer to [Log retriever traces](log-retriever-trace.md).

### Actions

From the Details view, you can also:

* **Share a trace:** Generate a public link to the trace. Refer to [Manage a trace](manage-trace.md#share-a-trace).
* **View server logs:** Access server logs associated with a trace generated by a LangSmith deployment. Refer to [Manage a trace](manage-trace.md#view-server-logs).
* **Add to a dataset:** Save the run as an example in a dataset for use in evaluations. Refer to [Manage datasets in the application](manage-datasets-in-application.md#manually-from-a-tracing-project).
* **Add to an annotation queue:** Send the run or its entire [thread](observability-concepts.md#threads) to a queue for human review and feedback. Refer to [Annotation queues](annotation-queues.md#assign-runs-and-threads-to-a-single-run-queue).
