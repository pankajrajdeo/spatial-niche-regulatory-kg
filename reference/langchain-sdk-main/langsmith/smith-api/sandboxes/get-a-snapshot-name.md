...
paths:
  /api/v2/sandboxes/snapshots-by-name/{name}:
    get:
      tags:
        - sandboxes
      summary: Get a snapshot name
      description: >-
        Get a snapshot name and every tag under it, with the snapshot each tag
        resolves to. To fetch one snapshot, use
        /api/v2/sandboxes/snapshots/{snapshot_id}.
      parameters:
        - description: Snapshot name, without a tag
          name: name
          in: path
          required: true
          schema:
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/sandboxes.SnapshotNameResponse'
        '400':
...
components:
  schemas:
    sandboxes.SnapshotNameResponse:
      type: object
      properties:
        name:
          type: string
        tags:
          type: array
          items:
            $ref: '#/components/schemas/sandboxes.SnapshotNameTag'
