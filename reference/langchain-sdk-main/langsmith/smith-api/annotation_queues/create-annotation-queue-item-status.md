...
description: "Log the caller's reviewer status for a RUN or THREAD annotation queue item. A null status re-shows the item for this reviewer."
source: "https://docs.langchain.com/langsmith/smith-api/annotation_queues/create-annotation-queue-item-status"
category: "docs"
tags: [docs, langsmith, smith-api, annotation_queues, create-annotation-queue-item-status]
---
...
tags:
  ...
  - name: productfeedback
    x-hidden: true
paths:
  /api/v1/platform/annotation-queues/items/{queue_item_id}/status:
    post:
      tags:
        - annotation_queues
      summary: Create annotation queue item status
      description: >-
        Log the caller's reviewer status for a RUN or THREAD annotation queue
        item. A null status re-shows the item for this reviewer.
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: >-
                #/components/schemas/annotationqueues.CreateAnnotationQueueItemStatusRequest
      responses:
        '200':
          description: OK
          content:
            application/json:
              schema:
                $ref: >-
                  #/components/schemas/annotationqueues.CreateAnnotationQueueItemStatusResponse
        '400':
...
components:
  schemas:
    annotationqueues.CreateAnnotationQueueItemStatusRequest:
      type: object
      properties:
        override_added_at:
          type: string
        status:
          $ref: '#/components/schemas/annotationqueues.AnnotationQueueReviewStatus'
    annotationqueues.CreateAnnotationQueueItemStatusResponse:
      type: object
      properties:
        is_archived:
          type: boolean
    ...
    shared.ProblemDetails:
      ...
      properties:
        ...
        type:
          type: string
    annotationqueues.AnnotationQueueReviewStatus:
      type: string
      enum:
        - viewed
        - completed
