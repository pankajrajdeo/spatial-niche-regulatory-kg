...
> [!NOTE]
> This page covers **LangSmith automation webhooks**, POST notifications triggered when automation rules match runs or threads. If you are looking for **LangGraph API webhooks** (a callback parameter you pass when creating or streaming a run), see [Use webhooks](use-webhooks.md).
...
* `"threads"`: this is an array of threads, sent in place of `"runs"` when the rule's [item type](rules.md#set-the-item-type-to-runs-or-threads) is **Threads**. A payload carries one array or the other, never both. For more information, refer to [Read a thread rule payload](#read-a-thread-rule-payload).
...
A rule whose [item type](rules.md#set-the-item-type-to-runs-or-threads) is **Threads** replaces the top-level `"runs"` array with a `"threads"` array. Each entry in that array contains:
...
> ```
>
> For the full filter syntax, refer to [Filter traces (ClickHouse)](filter-traces-in-application.md).
