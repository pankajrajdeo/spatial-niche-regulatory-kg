...
> [Agent Search on Gemini Enterprise Agent Platform](https://cloud.google.com/enterprise-search) (formerly known as `Enterprise Search` on `Generative AI App Builder`) is a part of the [Gemini Enterprise Agent Platform](https://cloud.google.com/products/gemini-enterprise-agent-platform) machine learning platform offered by `Google Cloud`.
>
> `Agent Search on Gemini Enterprise Agent Platform` lets organizations quickly build generative AI-powered search engines for customers and employees. It's underpinned by a variety of `Google Search` technologies, including semantic search, which helps deliver more relevant results than traditional keyword-based search techniques by using natural language processing and machine learning techniques to infer relationships within the content and intent from the user’s query input. Agent Search on Gemini Enterprise Agent Platform also benefits from Google’s expertise in understanding how users search and factors in content relevance to order displayed results.

> `Agent Search on Gemini Enterprise Agent Platform` is available in the `Google Cloud Console` and via an API for enterprise workflow integration.

This notebook demonstrates how to configure `Agent Search on Gemini Enterprise Agent Platform` and use the Agent Search on Gemini Enterprise Agent Platform [retriever](../../deepagents/retrieval.md). The Agent Search on Gemini Enterprise Agent Platform retriever encapsulates the [Python client library](https://cloud.google.com/generative-ai-app-builder/docs/libraries#client-libraries-install-python) and uses it to access the [Search Service API](https://cloud.google.com/python/docs/reference/discoveryengine/latest/google.cloud.discoveryengine_v1beta.services.search_service).
...
You need to install the `langchain-google-community` and `google-cloud-discoveryengine` packages to use the Agent Search on Gemini Enterprise Agent Platform retriever.

```python
pip install -qU langchain-google-community google-cloud-discoveryengine
```

### Configure access to Google Cloud and Agent Search

Agent Search on Gemini Enterprise Agent Platform is generally available without allowlist as of August 2023.
...
* Follow the instructions in the [Agent Search on Gemini Enterprise Agent Platform Getting Started guide](https://cloud.google.com/generative-ai-app-builder/docs/try-enterprise-search) to set up a Google Cloud project and Agent Search on Gemini Enterprise Agent Platform.
...
#### Set credentials to access the Agent Search API

The [Agent Search on Gemini Enterprise Agent Platform client libraries](https://cloud.google.com/generative-ai-app-builder/docs/libraries) used by the Agent Search on Gemini Enterprise Agent Platform retriever provide high-level language support for authenticating to Google Cloud programmatically.
...
### Configure and use the Agent Search retriever

The Agent Search on Gemini Enterprise Agent Platform retriever is implemented in the `langchain_google_community.VertexAISearchRetriever` class. The `get_relevant_documents` method returns a list of `langchain.schema.Document` documents where the `page_content` field of each document is populated the document content.
Depending on the data type used in Agent Search on Gemini Enterprise Agent Platform (website, structured or unstructured) the `page_content` field is populated as follows:
...
* `engine_data_type` - Defines the Agent Search on Gemini Enterprise Agent Platform data type
  * `0` - Unstructured data
  * `1` - Structured data
  * `2` - Website data
