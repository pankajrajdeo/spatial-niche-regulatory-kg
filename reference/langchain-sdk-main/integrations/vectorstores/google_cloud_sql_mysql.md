...
**Colab only:** Uncomment the following cell to restart the kernel or use the button to restart the kernel. For Gemini Enterprise Agent Platform Workbench you can restart the terminal using the button on top.
...
engine.init_vectorstore_table(
    ...
    vector_size=768,  # Vector size for Gemini Enterprise Agent Platform model (textembedding-gecko@latest)
)
```
...
You may need to enable the Gemini Enterprise Agent Platform API to use `VertexAIEmbeddings`.

We recommend pinning the embedding model's version for production, learn more about the [Text embeddings models](https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/models/text-embeddings-api).

```python
# enable Gemini Enterprise Agent Platform API
...
engine.init_vectorstore_table(
    ...
    vector_size=768,  # Gemini Enterprise Agent Platform model: textembedding-gecko@latest
