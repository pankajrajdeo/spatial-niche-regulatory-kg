...
Access Google's Generative AI models, including the Gemini family, via the **Gemini Developer API** or **Gemini Enterprise Agent Platform**. The Gemini Developer API offers quick setup with API keys, ideal for individual developers. Gemini Enterprise Agent Platform provides enterprise features and integrates with Google Cloud Platform.
...
> [!NOTE]
> **Gemini Enterprise Agent Platform consolidation & compatibility**
>
...
>
> This migration brings support for Gemini models both via the Gemini Developer API and Gemini API on Gemini Enterprise Agent Platform, superseding certain classes in `langchain-google-vertexai`, such as `ChatVertexAI`.
>
...
This integration supports two backends: **Gemini Developer API** and **Gemini Enterprise Agent Platform**. The backend is selected automatically based on your configuration.
...
2. If `credentials` parameter is provided, uses Gemini Enterprise Agent Platform
3. If `project` parameter is provided, uses Gemini Enterprise Agent Platform
...
#### Gemini Enterprise Agent Platform with API key
**Gemini Enterprise Agent Platform using API key authentication**

You can use Gemini Enterprise Agent Platform with API key authentication for simpler setup:
...
#### Gemini Enterprise Agent Platform with credentials
**Gemini Enterprise Agent Platform using service account or ADC**
...
| `GOOGLE_GENAI_USE_VERTEXAI` | Force Gemini Enterprise Agent Platform backend (`true`/`false`) | Gemini Enterprise Agent Platform       |
| `GOOGLE_CLOUD_PROJECT`      | GCP project ID                                                  | Gemini Enterprise Agent Platform       |
| `GOOGLE_CLOUD_LOCATION`     | GCP region (default: `us-central1`)                             | Gemini Enterprise Agent Platform       |
...
Providing `project` automatically selects the Gemini Enterprise Agent Platform backend unless you explicitly set `vertexai=False`.
...
> [!WARNING]
> **Gemini Enterprise Agent Platform limitation**
>
> Audio generation models are currently in limited preview on Gemini Enterprise Agent Platform and may require allowlist access. If you encounter an `INVALID_ARGUMENT` error when using TTS models with `vertexai=True`, your GCP project may need to be allowlisted.
>
...
Accepted values are `"standard"`, `"flex"`, and `"priority"`. The value is the same on both backends, and the integration sends it the way each backend expects: a request body field on the Gemini Developer API, a request header on Gemini Enterprise Agent Platform.
...
For more information, see the Gemini Developer API guides for [Flex](https://ai.google.dev/gemini-api/docs/flex-inference) and [Priority](https://ai.google.dev/gemini-api/docs/priority-inference), and the Gemini Enterprise Agent Platform guides for [Flex](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/flex-paygo) and [Priority](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/priority-paygo).
