...
tags: [blog, improving-deep-agents-with-harness-engineering]
---
...
At LangChain, we use [Traces](../langsmith/observability-quickstart.md) to understand agent failure modes at scale. Models today are largely black-boxes, their inner mechanisms are hard to interpret. But we can see their inputs and outputs in text space which we then use in our improvement loops.
...
An agent harness has a lot of knobs: system prompts, tools, hooks/middleware, skills, sub-agent delegation, memory systems, and more. We deliberately compress the optimization space and focus on three: **System Prompt, Tools,** and [**Middleware**](../langchain/middleware/overview.md#the-agent-loop) (our term for hooks around model and tool calls).
...
In a multi-model harness, balancing reasoning budgets could play out as using a large model for planning and [handing off](../langchain/multi-agent/handoffs.md) to a smaller model for implementation.
