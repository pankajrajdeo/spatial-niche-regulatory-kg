...
description: "A practical guide to agent memory, including short-term and long-term memory, trace analysis, and how LangSmith helps agents improve across runs."
source: "https://www.langchain.com/blog/how-to-give-your-agent-memory"
category: "blog"
published: "2026-06-24T16:00:00.000Z"
author: "LangChain Accounts"
tags: [blog, how-to-give-your-agent-memory]
---

# How To Give Your Agent Memory

Building a [loop](https://www.langchain.com/blog/the-art-of-loop-engineering) so that your agent can learn from previous actions allows you to create delightful agentic experiences that learn with the user. This is often called** **[**memory**](../concepts/memory.md). Memory can be the difference between your users having to constantly repeat corrections instead of the agent remembering how to do something correctly after the first time it is told.
...
- [LangSmith Observability](../langsmith/observability.md) as a trace store
- [LangSmith Engine](../langsmith/engine.md) as a process that analyzes traces
- [LangSmith Context Hub](../langsmith/use-the-context-hub.md) as a memory store
...
A [trace](../langsmith/observability-concepts.md#traces), transcript, or log is useful evidence of what happened. It becomes memory only when the relevant lesson is converted into context the agent can retrieve on a later run and use to change its behavior.
...
[**Short-term memory**](../concepts/memory.md#short-term-memory) is the context available while the agent is doing the task in front of it: the current thread, recent messages, tool results, retrieved documents, intermediate reasoning artifacts, and temporary files or state the agent needs to finish the current job.

[**Long-term memory**](../concepts/memory.md#long-term-memory) is context that persists beyond the current run: facts, preferences, examples, workflows, policies, instructions, and skills that should be available later to shape the agent’s behavior over time.
...
- [**Semantic memory**](../concepts/memory.md#semantic-memory) is what the agent knows: facts, preferences, and general knowledge.
- [**Episodic memory**](../concepts/memory.md#episodic-memory) is what the agent has experienced: past interactions, examples, actions, and outcomes.
- [**Procedural memory**](../concepts/memory.md#procedural-memory) is how the agent should behave: instructions, workflows, policies, skills, and tool-use rules.
