...
category: "docs"
tags: [docs, api-reference, deployments-v2, patch-deployment]
---
...
components:
  schemas:
    DeploymentPatchRequest:
      properties:
        display_name:
          ...
          title: Display Name
          description: >-
            Human-readable display name for the deployment. Updating this does
        ...
        tracked_repo_ref:
          anyOf:
            - type: string
              minLength: 1
            - type: 'null'
          title: Tracked Repo Ref
          description: >-
            Retarget the git ref the deployment tracks without creating a new
            revision. Use `source_revision_config.repo_ref` to deploy from a ref
            instead.
        deployment_tier:
          anyOf:
        ...
        route_through_gateway:
          ...
          description: >-
            Enable or disable routing OpenAI and Anthropic model calls through
            the LangSmith LLM gateway.
      type: object
    ...
    SourceConfigRequest:
      ...
      description: >-
        Deployment configuration fields specific to the `source`.<br>

        <br>

        All fields are nullable, but are validated based on the `source`.

        Updating `resource_spec`, `install_command`, or `build_command` creates
        a new revision.
