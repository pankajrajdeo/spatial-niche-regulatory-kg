---
title: "Patch Deployment Resource Tiers"
description: "Patch a deployment's compute and database resource tiers independently."
source: "https://docs.langchain.com/api-reference/deployments-v2/patch-deployment-resource-tiers"
category: "docs"
tags: [docs, api-reference, deployments-v2, patch-deployment-resource-tiers]
---
...
security: []
paths:
  /v2/deployments/{deployment_id}/resource-tiers:
    patch:
      tags:
        - Deployments (v2)
      summary: Patch Deployment Resource Tiers
      description: Patch a deployment's compute and database resource tiers independently.
      operationId: >-
        patch_deployment_resource_tiers_v2_deployments__deployment_id__resource_tiers_patch
      parameters:
        - name: deployment_id
          in: path
          required: true
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/DeploymentResourceTiersUpdateRequest'
      responses:
        '200':
...
components:
  schemas:
    DeploymentResourceTiersUpdateRequest:
      properties:
        compute_tier:
          anyOf:
          ...
          title: Compute Tier
          description: >-
            Fixed resource tier to apply to the deployment's compute. Serverless
            deployments support `SERVERLESS_S`, `SERVERLESS_M`, and
            `SERVERLESS_L`; dedicated deployments support `DEDICATED_S`,
            `DEDICATED_M`, and `DEDICATED_L`. Leave `null` to keep the current
            compute tier.
        database_tier:
          anyOf:
            - $ref: '#/components/schemas/DeploymentTier'
          ...
          title: Database Tier
          description: >-
            Fixed resource tier to apply to the deployment's database. Only
            supported for dedicated deployments (`DEDICATED_S`, `DEDICATED_M`,
            `DEDICATED_L`). Leave `null` to keep the current database tier.
      type: object
      title: DeploymentResourceTiersUpdateRequest
      description: >-
        Request body for updating a deployment's compute and database tiers
        independently.

        At least one of `compute_tier` or `database_tier` must be provided. A
        field

        left `null` keeps the deployment's current tier for that resource.
    Deployment:
      properties:
        name:
          anyOf:
          ...
          title: Name
          description: >-
            Deployment name. Required when agent is omitted; retained as the
            agent identity in the production environment when agent associations
            are enabled. Omit when supplying agent to generate a deployment
            name.
        agent:
          anyOf:
            - $ref: '#/components/schemas/DeploymentAgent'
            - type: 'null'
          description: >-
            Agent and environment to deploy. Both fields are required when
            supplied.
        source:
          type: string
          enum:
        ...
        shareable:
          ...
          title: Shareable
          description: >-
            Whether the deployment can be shared with LangSmith users outside
            the organization.
          default: false
        route_through_gateway:
          type: boolean
          title: Route Through Gateway
          description: >-
            Route OpenAI and Anthropic model calls through the LangSmith LLM
            gateway.
          default: false
        id:
          type: string
        ...
        tracer_session_id:
          anyOf:
            - type: string
              format: uuid
            - type: 'null'
          title: Tracer Session ID
          description: >-
            The ID of the LangSmith tracing project the deployment writes runs
            to.<br><br>This is a read-only field.
        url:
          anyOf:
            - type: string
            - type: 'null'
          title: URL
          description: >-
            The deployment's serving URL, once provisioned.<br><br>This is a
            read-only field.
        is_preview:
          type: boolean
          title: Is Preview
          description: >-
            Whether this is a preview-build deployment. Full preview metadata
            (parent deployment, PR, lifecycle) is available from the
            preview-builds details endpoint.<br><br>This is a read-only field.
          default: false
        image_version:
          anyOf:
            - type: string
        ...
        is_managed_deep_agent:
          type: boolean
          title: Is Managed Deep Agent
          description: >-
            Whether this deployment is a Managed Deep Agent
            deployment.<br><br>This is a read-only field.
          default: false
        deployment_tier:
          anyOf:
          ...
          description: >-
            The selected fixed resource tier for this deployment. Existing
            deployments without an explicit selection return the default tier
            for their deployment type. When compute and database tiers differ,
            this reflects the compute tier.
        compute_tier:
          anyOf:
            - $ref: '#/components/schemas/DeploymentTier'
            - type: 'null'
          title: Compute Tier
          description: >-
            The selected fixed resource tier for this deployment's compute.
            Falls back to `deployment_tier` for deployments predating
            independent compute/database scaling.
        database_tier:
          anyOf:
            - $ref: '#/components/schemas/DeploymentTier'
            - type: 'null'
          title: Database Tier
          description: >-
            The selected fixed resource tier for this deployment's database.
            Falls back to `deployment_tier` for deployments predating
            independent compute/database scaling.
      type: object
      required:
    ...
    DeploymentAgent:
      ...
      description: Agent identity and environment for a deployment.
    SourceConfig:
      properties:
        integration_id:
          anyOf:
        ...
        install_command:
          ...
          description: >-
            ...
            applies to JS deployments using 'github' or 'internal_source'.
            Updating this field creates a new revision. This command will be run
            from the root of your repository - NOT where your langgraph.json
            file lives (unless these are the same place). If not provided, we
            will autodetect the command based on the presence of yarn/npm/etc.
            files.
        build_command:
          anyOf:
            - type: string
          ...
          description: >-
            ...
            applies to JS deployments using 'github' or 'internal_source'.
            Updating this field creates a new revision. This command will be run
            from where your langgraph.json file lives. This is helpful for using
            shared workspaces in your deployments, without having to provide a
        ...
        repo_branch:
          ...
          title: Repo branch
          description: >-
            The git branch the deployment tracks for new revisions.<br><br>This
            is a read-only field. Only applicable for `github` source.
      type: object
      title: SourceConfig
    ...
    ResourceSpec-Output:
      properties:
        ...
        core_cpu:
          ...
          title: Core API CPU
          description: >-
            CPU request in cores for the core API process (only used when the
            deployment runs the core API as a sidecar). If core_cpu_limit is not
            set, limit defaults to 2x this value.
        core_cpu_limit:
          anyOf:
            - type: number
              minimum: 0.1
            - type: 'null'
          title: Core API CPU Limit
          description: >-
            CPU limit in cores for the core API process. If not set, defaults to
            2x core_cpu.
        core_memory_mb:
          anyOf:
            - type: integer
            - type: 'null'
          title: Core API Memory (MB)
          description: >-
            Memory request (MB) for the core API process. If
            core_memory_limit_mb is not set, limit defaults to 2x this value.
        core_memory_limit_mb:
          anyOf:
            - type: integer
            - type: 'null'
          title: Core API Memory Limit (MB)
          description: >-
            Memory limit (MB) for the core API process. If not set, defaults to
            2x core_memory_mb.
        labels:
          anyOf:
