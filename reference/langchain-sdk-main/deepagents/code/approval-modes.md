...
category: "docs"
tags: [docs, deepagents, code, approval-modes]
---
...
Press `Shift+Tab` to cycle YOLO → Manual → Auto → YOLO. Set [`startup.yolo_switcher = false`](config-file.md#startup-approval-mode) to omit YOLO from the cycle.
...
When no flag or configured mode applies, Deep Agents Code restores the last selected Manual or Auto mode. YOLO must be explicitly selected. See [Startup approval mode](config-file.md#startup-approval-mode).

For flag and configuration precedence, see [Startup approval mode](config-file.md#startup-approval-mode).
